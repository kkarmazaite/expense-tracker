...
export default defineEventHandler( async (event: H3Event) => {
    ...
    if(accountCategories.length>0){
        return sendError(event, createError({statusCode: 400, statusMessage: 'Cannot delete account, because it has categories assigned to it.'}))
    }

    const account:IAccount | null = await getAccountById(accountId)

    if(!account){
        return sendError(event, createError({statusCode: 400, statusMessage: 'Account not found'}))
    }
